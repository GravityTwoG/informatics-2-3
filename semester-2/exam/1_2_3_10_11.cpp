...
ListNode* findMax(ListNode* head, ListNode*& behindMax) {
  if (head == NULL) return NULL;
  ListNode* max = head;
  ListNode* currentNode = max->next;
  ListNode* prevNode = max;
...
void selectionSort(ListNode*& head) {
  ...
  while (currentNode != NULL) {
    ListNode* behindMax = NULL;
    ListNode* maxNode = findMax(currentNode, behindMax);
    if (maxNode == NULL) break;

    // Вырезаем элемент из списка
    if (behindMax != NULL && maxNode->next != NULL) { // из середины
      behindMax->next = maxNode->next;
      currentNode = head;
    }  else if (behindMax != NULL) {                  // из конца списка
      behindMax->next = NULL;
      currentNode = head;
    } else if (maxNode->next != NULL) {               // из начала списка
      head = maxNode->next;
      currentNode = head;
    } else {                    // в изначальном списке остался 1 элемент 
      head = NULL;
      currentNode = NULL;
    }
...
void insertNode(ListNode *&head, ListNode* newNode) { 
  ...
  if (prevNode == NULL) {// включение в начало списка
    head = newNode;
...
ListNode* removeNode(ListNode *&head, int n) { 
  ...
  for (
    currentNode = head; 
    currentNode != NULL && n != 0; 
    prevNode = currentNode, currentNode = currentNode->next, n--
  );  // Отсчитать n-ый
  if (currentNode == NULL) return NULL;  // нет элемента с таким номером
  if (prevNode == NULL)         // удаление первого
...
// Создаёт односвязный список необходмой длины
ListNode* createList(int nodesAmount) {
  ListNode* head = NULL;
